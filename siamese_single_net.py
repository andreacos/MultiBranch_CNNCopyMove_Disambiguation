...
class SiameseSingleNet:
    def __init__(self, use_tf_threading=False, batch_size=64, im_size=64, \
        ...
        if self.use_tf_threading:
            ...
            self.label  = tf.placeholder_with_default(label, [None, 1])
        else:
                ...
                with tf.device('/gpu:%d' % gpu_id):
                    with tf.name_scope('tower_%d' % gpu_id):
                        ...
                        with tf.name_scope('classification_loss'):
                            loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=labels, logits=out_ab)
    ...
    def predict(self, feat_ab, name, reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            ...
            out = layers.fully_connected(out, 1, activation_fn=None, scope='fc_out')
        return out

def initialize(args):
    return SiameseSingleNet(**args)