...
class SingleNetSolver:
    ...
    def validate_single(self):
        ...
        for i in range(n_val_batches):
            if i != n_val_batches-1:
                data = self.data.data_val(offset, batch_size)
                offset += batch_size
            else:
                if self.data.n_val > offset + batch_size:
                    data = self.data.data_val(offset, batch_size)
                else:
                    data = self.data.data_val(offset, self.data.n_val-offset)
            _,pred_cls = self.predict_single(data[0], data[1])
            if i == 0:
                labels = data[2]
                predictions = pred_cls
            else:
                labels = np.concatenate((labels, data[2]))
                predictions = np.concatenate((predictions, pred_cls))
            
            tmp_acc = np.float(np.sum(data[2].astype(int) == pred_cls.astype(int))) / data[2].shape[0]

        labels = labels.astype(int)
        predictions = predictions.astype(int)

        acc = np.sum(predictions == labels, dtype=np.float32) / labels.shape[0]
        print('Acc %f' % acc)
        return 1 - acc
    
    def predict_single(self, im_a, im_b):
        if im_a.ndim < 4:
            im_a, im_b = (im_a[np.newaxis,...], im_b[np.newaxis,...])
    ...
    def _train_single(self):
        if self.net.use_tf_threading:
            ...
            data = self.data.data_train(self.batch_size)
            [_, loss] = self.sess.run([self.net.opt, self.net.loss], feed_dict={
                self.net.im_a:data[0],
                self.net.im_b:data[1],
                self.net.label:data[2],
                self.net.is_training:True,
                self.net.learning_rate:self.learning_rate
            })
        return loss
