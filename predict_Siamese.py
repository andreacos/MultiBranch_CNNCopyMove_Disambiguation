...
def predict_with_mask(predictor, im, mask):
    ...
    if (src_coords is not None and dst_coords is not None):
        I_dst, I_dst_est, I_src, I_src_est = compute_pair_from_coords(im, T, src_coords, dst_coords, crop_position='corner')
        ...
        soft_pred,pred_cls = predictor.predict_single(I_dst/255, I_dst_est/255)

        # maximum soft scores
        confi= np.argmax(np.abs(0.5-soft_pred[:,0]))
        output_mask[src_coords[1,:], src_coords[0,:], int(pred_cls[confi,0])] = 255
        output_mask[dst_coords[1,:], dst_coords[0,:], 1-int(pred_cls[confi,0])] = 255
        return output_mask
    else:
        return None
...
if __name__ == "__main__":
    ...
    net.model()
    predictor = single_net_solver.initialize({
    ...
    for img_path,mask_path in zip(images, masks):
        ...
        if pred_mask is not None:
            ...
            cv.imwrite(mask_path.replace('_tm', '_siamese_pred'), pred_mask)
        else:
